...
class HomeCubit extends CubitBase<HomeState> {
  HomeCubit(this._getQrCodesStreamUseCase, this._qrCodeRepository,
      this._geoPositionRepository)
      : super(HomeState()) {
  ...
  Future<void> loadNearestQrCode(final BuildContext context) async {
    try {
      final Either<BaseException, Position?> result =
          await executeAsync(_geoPositionRepository.determinePosition());
      final Position? position = result.fold(
        (final error) {
      ...
      if (position != null) {
        final List<QrCodeEntity> nearbyQrCodes =
            (await _qrCodeRepository.getNearbyQrCodes(
          position.latitude,
          position.longitude,
        ))
                .fold(
